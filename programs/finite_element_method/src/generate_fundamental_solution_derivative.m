...
fundamentalSolutionDerivative = ...
    transpose(2*distanceX.*distanceY./(distanceX.^2+distanceY.^2).^2);
...
WaveFundamentalSolutionDerivative = ...
    solutions*borderControlBasisCoefficients;

tri = delaunay(model.Mesh.Nodes(1,:),model.Mesh.Nodes(2,:));
trimesh(tri, ...
    model.Mesh.Nodes(1,:),model.Mesh.Nodes(2,:), ...
    WaveFundamentalSolutionDerivative)
